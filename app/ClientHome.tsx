    ...
    <div className="min-h-screen bg-gray-950 text-gray-100 font-sans">
      <header className="border-b border-gray-800 bg-gray-900/50 backdrop-blur px-6 py-4 flex items-center justify-between sticky top-0 z-50">
        <div className="flex items-center gap-2 cursor-pointer" onClick={() => { setSelectedSeries(null); setCurrentChapterIndex(-1); }}>
          ...
          <h1 className="text-xl font-bold bg-gradient-to-r from-indigo-400 to-purple-400 bg-clip-text text-transparent">PG-Archive</h1>
        </div>
      ...
      <main className="max-w-6xl mx-auto p-4 sm:p-6">
        ...
        {currentChapter ? (
          <div className="space-y-4">
            <button onClick={() => setCurrentChapterIndex(-1)} className="flex items-center gap-1 text-sm text-gray-400 hover:text-white mb-2">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 bg-gray-900 p-4 rounded-xl border border-gray-800">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-semibold text-gray-300 px-2">Cap. {currentChapter.number}</span>
            ...
            <div className="relative flex flex-col items-center bg-gray-900 rounded-xl p-2 border border-gray-800">
              {currentChapter.pages.length > 0 ? (
                <div className="relative group max-w-3xl w-full flex justify-center">
                  <img src={currentChapter.pages[currentPageIndex]} alt={`Página ${currentPageIndex + 1}`} className="max-h-[85vh] object-contain rounded shadow-2xl" />
              ...
              <p className="text-xs text-gray-500 mt-2">Página {currentPageIndex + 1} de {currentChapter.pages.length}</p>
            </div>
          </div>
        ) : selectedSeries ? (
            ...
            {(role === 'Admin' || role === 'Scan') && (
              <div className="bg-gray-900 border border-gray-800 p-4 rounded-xl space-y-3">
                ...
                <form onSubmit={handleUploadChapter} className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <input type="number" placeholder="Núm. Capítulo" value={chapterNumber} onChange={(e) => setChapterNumber(Number(e.target.value))} required className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-1.5 text-sm" />
                  <input type="text" placeholder="Título opcional" value={chapterTitle} onChange={(e) => setChapterTitle(e.target.value)} className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-1.5 text-sm" />
                  <input type="file" multiple accept="image/*,.cbz,.cbr,.zip,.rar" onChange={(e) => setChapterFiles(e.target.files)} required className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-1 text-sm text-gray-300 file:mr-2 file:py-1 file:px-2 file:rounded file:border-0 file:bg-indigo-600 file:text-white" />
                  <button type="submit" disabled={uploading} className="md:col-span-3 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 py-2 rounded-lg font-medium text-sm transition">{uploading ? 'Subiendo páginas...' : 'Publicar Capítulo'}</button>
                </form>
              </div>
            )}
              ...
              {chapters.length === 0 ? (
                ...
                <div className="grid gap-2">
                  {chapters.map((chap, idx) => (
                    <div key={chap.id} onClick={() => { setCurrentChapterIndex(idx); setCurrentPageIndex(0); }} className="flex items-center justify-between p-3 bg-gray-900 hover:bg-gray-800 rounded-lg border border-gray-800 cursor-pointer transition">
                      <div>
          ...
          <div className="space-y-6">
            {(role === 'Admin' || role === 'Scan') && (
              <div className="bg-gray-900 border border-gray-800 p-4 rounded-xl space-y-3">
                ...
                <form onSubmit={handleCreateSeries} className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <input type="text" placeholder="Título de la obra" value={newSeriesTitle} onChange={(e) => setNewSeriesTitle(e.target.value)} required className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-1.5 text-sm" />
                  <input type="text" placeholder="URL de la portada" value={newSeriesCover} onChange={(e) => setNewSeriesCover(e.target.value)} className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-1.5 text-sm" />
                  <input type="text" placeholder="Sinopsis / Descripción" value={newSeriesDesc} onChange={(e) => setNewSeriesDesc(e.target.value)} className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-1.5 text-sm" />
                  <button type="submit" className="md:col-span-3 bg-indigo-600 hover:bg-indigo-500 py-2 rounded-lg font-medium text-sm transition">Crear Serie</button>
                </form>
              </div>
            )}
            ...
            {seriesList.length === 0 ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                {seriesList.map((s) => (
                  <div key={s.id} onClick={() => openSeries(s)} className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden hover:border-indigo-500/50 cursor-pointer transition group">
